...
def add_task(task, creator_id, doer_id, deadline):
    sql = "INSERT INTO" \
            " tasks" \
                " (task, creator_id, doer_id, deadline)"\
            " VALUES" \
                " (:task, :creator_id, :doer_id, :deadline)"
...
def count_today_done_tasks_by_user(doer_id, today):
    sql = "SELECT COUNT(task) FROM tasks WHERE doer_id=:doer_id AND deadline=:deadline AND done = true"
    result = db.session.execute(sql, {"doer_id":doer_id, "deadline":today})
    tasks = result.fetchall()
    if tasks is None:
        return False
...
def count_all_done_tasks(user_id, role, today=0):
    sql = "SELECT COUNT(task) FROM tasks WHERE creator_id=:user_id done = true"
    result = db.session.execute(sql, {"creator_id":user_id, "deadline":today})
    tasks = result.fetchall()
    if tasks is None:
        return False
...
def count_all_tasks_by_status(user_id, role, status):
    sql = "SELECT COUNT(task) FROM tasks WHERE creator_id=:user_id AND status=:status"
    result = db.session.execute(sql, {"creator_id":user_id, "status":status})
    tasks = result.fetchall()
    if tasks is None:
        return False
    return tasks
